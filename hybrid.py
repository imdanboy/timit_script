...
class MyLuongAttention(tf.contrib.seq2seq.LuongAttention):
    def __init__(self,
        ...
        self._scale = scale
        self._name = name
        self.align_conv = layers_conv.Conv1D(filters=256, kernel_size=201, strides=1, padding='same', activation=tf.tanh)
        self.align_dense = layers_core.Dense(1)
        #self.align_hist = tf.zeros(tf.shape(memory)[2:], dtype=tf.float32)
    ...
    def __call__(self, query, state):
        with tf.variable_scope(None, "luong_attention", [query]):
            ...
            conv_feat = tf.squeeze(conv_feat, -1)
            score = _luong_score(query, self._keys, self._scale)
            score = score + conv_feat
