...
def _luong_score(query, keys, scale, conv_feat):
    depth = query.get_shape()[-1]
    key_units = keys.get_shape()[-1]
    if depth != key_units:
        raise ValueError()
    query = tf.expand_dims(query, 1) # [batch, 1, num_decoder]
    keys = keys + conv_feat # [batch, max_time, num_units(num_decoder)]
...
class MyLuongAttention(tf.contrib.seq2seq.LuongAttention):
    def __init__(self,
        ...
        self._scale = scale
        self._name = name
        self.align_conv = layers_conv.Conv1D(filters=32, kernel_size=129, strides=1, padding='same')
        self.align_dense = layers_core.Dense(num_units)
        self.lookahead_dense = layers_core.Dense(num_units)

    def __call__(self, query, state, right=None):
        with tf.variable_scope(None, "luong_attention", [query]):
            if right:
                state1, state2 = state
                state1, state2 = tf.expand_dims(state1, -1), tf.expand_dims(state2, -1)
                conv_feat1 = tf.scalar_mul(0.5, self.align_conv(state1))
                conv_feat2 = tf.scalar_mul(0.5, self.align_conv(state2))
                conv_feat = self.align_dense(conv_feat1 + conv_feat2)
            else:
                expanded_state = tf.expand_dims(state, -1)
                conv_feat = self.align_conv(expanded_state)
                conv_feat = self.align_dense(conv_feat)
            score = _luong_score(query, self._keys, self._scale, conv_feat)
...
def _compute_attention(attention_mechanism, cell_output, attention_state,
    ...
    lookahead_query = attention_mechanism.lookahead_dense(context)  # [batch, num_decoder]
    lookahead_align, _ = attention_mechanism(lookahead_query, state=(attention_state, alignments), right=True)
    expanded_lookahead_align = tf.expand_dims(lookahead_align, 1)
    lookahead_context = tf.matmul(expanded_lookahead_align, attention_mechanism.values)
    lookahead_context = tf.squeeze(lookahead_context, [1])

    if attention_layer is not None:
        raise NotImplementedError()
    else:
        attention = (context, lookahead_context)
...
class MyAttentionWrapper(tf.contrib.seq2seq.AttentionWrapper):
    ...
    def __init__(self, cell, attention_mechanism, keep_prob, training, attention_layer_size=None, alignment_history=False, cell_input_fn=None,
        ...
        self.context_dense = layers_core.Dense(256)
        self.lookahead_dense = layers_core.Dense(256)
    def call(self, inputs, state):
        ...
        for i, attention_mechanism in enumerate(self._attention_mechanisms):
            (attention, lookahead_context), alignments, next_attention_state = _compute_attention(
        ...
        attention = self.context_dense(attention)
        lookahead_context = self.lookahead_dense(lookahead_context)
        outputs = layers_core.Dropout(rate=1-keep_prob)(self.attention_layer(tf.concat([cell_output, attention, lookahead_context], 1)), training=self.training)
...
def model_fn(features, labels, mode, params):
    ...
    def compute_encoder_outputs():
        def residual_block(inp, out_channels):
            ...
            if inp_channels != out_channels:
                ...
                inp = tf.layers.batch_normalization(inp, axis=1, training=training, scale=False)
            return out + inp
    ...
    def get_decoder_cell_and_init_state():    
        ...
        decoder_cell = tf.nn.rnn_cell.DropoutWrapper(tf.contrib.cudnn_rnn.CudnnCompatibleLSTMCell(num_decoder), output_keep_prob=keep_prob)
        #decoder_cell = tf.nn.rnn_cell.DropoutWrapper(tf.contrib.rnn.LSTMCell(num_decoder, use_peepholes=True, initializer=tf.random_uniform_initializer(minval=-0.1, maxval=0.1)),
